...
const targetStr = `                    <div className="flex-1 overflow-y-auto divide-y divide-slate-150">\n                      {(() => {\n                        const filtered = gmailList.filter(mail => {`;

const replacement = `                    <div className="flex-1 overflow-y-auto divide-y divide-slate-150">
                      {(() => {
                        if (needsAuth) {
                           return (
...
if (code.includes(targetStr)) {
    ...
    console.log("Could not find UI target string");
}
